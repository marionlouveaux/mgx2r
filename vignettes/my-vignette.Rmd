...
plotlyMesh <- function(meshExample,
  ...
  if (!is.null(nrow(meshColors)) && ncol(meshColors)>1){
    ...
    if (is.null(meshColors)){
      color <- rep("#00FFFF", ncol(myMesh$it))
    }else{
      color <- meshColors
    }
  }
  ...
  if (!is.null(meshCellcenter)){
    p <- add_markers(p, x = meshCellcenter$x,
                     y = meshCellcenter$y,
                     z = meshCellcenter$z,
                     marker=list(symbol = 'circle', sizemode="diameter", sizeref = 0.01),
                     sizes = c(0.0005, 0.0005) ## PB with size!!!
    ) 
  }
...
plotlyMesh(meshExample = myMesh, 
           meshColors = left_join(myMesh$it_label, myCellGraph$vertices) %>% select(., GeometryArea) %>% t(.),
           meshCellcenter = NULL)
```
